...
async def process_menu_background(session_id: str, file_path: str, use_phase2_integration: bool = True):
    """
    バックグラウンドでメニュー処理を実行
    ...
    Args:
        ...
        use_phase2_integration: Phase 2統合処理を使用するか（デフォルト: True）
    """
    ...
    # 必要な関数が利用できない場合のチェック
    if not all([stage1_ocr, stage2_categorize, send_progress]):
    ...
    try:
        ...
        if use_phase2_integration:
            # 🚀 Phase 2統合処理（翻訳→詳細説明→画像生成）
            await process_phase2_integration(
                session_id=session_id,
                categorized_data=stage2_result["categories"],
                send_progress=send_progress
            )
        else:
            # 🔄 従来の分離ステージ処理（Stage 3→4→5）
            await process_traditional_stages(
                session_id=session_id,
                categorized_data=stage2_result["categories"],
                stage3_translate=stage3_translate,
                stage4_descriptions=stage4_descriptions,
                stage5_images=stage5_images,
                send_progress=send_progress,
                combine_menu_with_images=combine_menu_with_images
            )
...
async def process_phase2_integration(session_id: str, categorized_data: dict, send_progress):
    """Phase 2統合処理（翻訳→詳細説明→画像生成）"""
    
    try:
        from app.services.translation import translate_menu_realtime
        
        total_items = sum(len(items) for items in categorized_data.values())
        
        # 統合処理開始通知
        await send_progress(session_id, 3, "active", f"🚀 Phase 2統合処理開始: {total_items}アイテムを3つずつ統合処理中...", {
        ...
        # Phase 2統合処理実行
        result = await translate_menu_realtime(
            categorized_data=categorized_data,
            session_id=session_id,
            mode="phase2_integration"
        )
        
        if result.success:
            # 統合処理成功
            completed_items = sum(len(items) for items in result.translated_categories.values())
            
            # Stage 3-5完了通知（統合処理として）
            await send_progress(session_id, 3, "completed", "✅ 翻訳完了（統合処理）", {
                "translated_categories": result.translated_categories,
                "processing_mode": "phase2_integration",
                "show_translated_menu": True
            })
            
            await send_progress(session_id, 4, "completed", "✅ 詳細説明完了（統合処理）", {
                "final_menu": result.translated_categories,
                "processing_mode": "phase2_integration"
            })
            
            await send_progress(session_id, 5, "completed", f"🎨 画像生成完了（統合処理）: {completed_items}アイテム処理完了", {
                "images_generated": True,
                "final_menu_with_images": result.translated_categories,
                "processing_mode": "phase2_integration",
                "total_items": completed_items,
                "integration_features": result.metadata.get("integration_features", [])
            })
            
            # 最終完了通知
            await send_progress(session_id, 6, "completed", "🎉 Phase 2統合処理完了！全ての機能が統合実行されました", {
                "processing_summary": {
                    "ocr_success": True,
                    "categorization_success": True,
                    "integration_success": True,
                    "completion_type": "phase2_integration",
                    "total_items_processed": completed_items,
                    "processing_method": "three_item_integration",
                    "integration_features": result.metadata.get("integration_features", [])
                }
            })
            
        else:
            # 統合処理失敗時は従来方式にフォールバック
            await send_progress(session_id, 3, "error", f"Phase 2統合処理エラー: {result.error}。従来方式にフォールバック中...")
            
            # フォールバック実行
            from app.workflows.stages import stage3_translate_with_fallback, stage4_add_descriptions, stage5_generate_images
            from app.services.image import combine_menu_with_images
        ...
        await send_progress(session_id, 3, "error", f"Phase 2統合処理エラー: {str(e)}")

async def process_traditional_stages(session_id: str, categorized_data: dict, stage3_translate, stage4_descriptions, stage5_images, send_progress, combine_menu_with_images):
    """従来の分離ステージ処理（Stage 3→4→5）"""
    
    # Stage 3: 翻訳（Google Translate + OpenAI フォールバック）
    total_categories_to_translate = len(categorized_data)
    total_items_to_translate = sum(len(items) for items in categorized_data.values())
    ...
    stage3_result = await stage3_translate(categorized_data, session_id)
    ...
    await send_progress(session_id, 6, "completed", "全ての処理が完了しました！", {
        "processing_summary": {
            ...
            "completion_type": "traditional" if all([stage4_result["success"], stage5_result["success"]]) else "partial",
