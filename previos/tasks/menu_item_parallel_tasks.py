...
def real_translate_menu_item(self, session_id: str, item_id: int, item_text: str, category: str = "Other"):
    ...
    try:
        ...
        # Google Providerサービスをインポート
        from app.services.providers.google import GoogleProviderService
        
        google_provider = GoogleProviderService()
        
        if not google_provider.is_translate_available():
            # フォールバックでOpenAI プロバイダーを試行
            try:
                from app.services.providers.openai import OpenAIProviderService
                openai_provider = OpenAIProviderService()
                
                # OpenAI プロバイダーに translate_menu_item がない場合はスキップ
                logger.info(f"⚠️ [REAL] Google Translate unavailable, using fallback")
                raise Exception("Google Translate unavailable, fallback needed")
            ...
            english_text = await_sync(google_provider.translate_menu_item(item_text))
            provider = "Google Translate API (Provider)"
...
def real_generate_menu_description(self, session_id: str, item_id: int, japanese_text: str, english_text: str = "", category: str = "Other"):
    ...
    try:
        ...
        if not openai_provider.is_available():
            ...
            # OpenAI Provider で詳細説明を生成
            result = openai_provider.generate_description(japanese_text, english_text, category)
            
            if result.get('success'):
                description = result.get('description', '')
                provider = "OpenAI Provider (GPT-4.1-mini)"
            else:
                # エラー時のフォールバック
...
def real_generate_menu_image(self, session_id: str, item_id: int, english_text: str, description: str, category: str = "Other"):
    ...
    try:
        ...
        if not google_provider.is_imagen_available():
            # フォールバック画像URL
            ...
            # Imagen 3 API で画像生成
            result = await_sync(google_provider.generate_single_image(
            ...
            if result.get('generation_success'):
                ...
                provider = "Google Provider (Imagen 3)"
                fallback_used = False
            else:
                # エラー時のフォールバック
                image_url = f"https://placeholder-images.example.com/food/{category.lower()}/{session_id}_{item_id}.jpg"
