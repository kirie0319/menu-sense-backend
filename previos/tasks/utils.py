...
# Redis接続（同期・非同期両対応）
try:
    # 同期Redisクライアント（Celeryタスク用）
    redis_client = redis.from_url(celery_settings.redis_url)
    
    # 非同期Redisクライアント（FastAPIサービス用）
    import redis.asyncio as redis_async
    async_redis_client = redis_async.from_url(celery_settings.redis_url)
...
def create_image_chunks(final_menu: Dict[str, List[Dict]], chunk_size: int = 3, min_chunks: int = None) -> List[Dict]:
    ...
    if min_chunks is None:
        min_chunks = min(processing_settings.image_concurrent_chunk_limit, total_items)
    
    # 強制的なワーカー均等活用モード
    if celery_settings.force_worker_utilization and celery_settings.dynamic_chunk_sizing:
        # 理想的なチャンク数を計算（ワーカー数の最小倍数）
        ideal_chunks = max(
            processing_settings.image_concurrent_chunk_limit,  # 最低でもワーカー数分
            int(processing_settings.image_concurrent_chunk_limit * celery_settings.min_chunks_per_worker)  # ワーカーあたりの最小チャンク数
        )
        
        # アイテム数が多い場合は効率性も考慮
        if total_items > processing_settings.image_concurrent_chunk_limit * 2:
        ...
        # 従来のロジック
        if total_items <= processing_settings.image_concurrent_chunk_limit:
            # アイテム数がワーカー数以下の場合：1アイテム1チャンク
            optimal_chunk_size = 1
        else:
            # 全ワーカーを均等に使用するためのチャンクサイズ
            optimal_chunk_size = max(1, total_items // processing_settings.image_concurrent_chunk_limit)
            # 基本チャンクサイズも考慮
    ...
    for category, items in final_menu.items():
        ...
        for i in range(0, len(items), optimal_chunk_size):
            ...
            chunk_info = {
                ...
                "force_utilization": celery_settings.force_worker_utilization
            }
    ...
    if celery_settings.force_worker_utilization and len(chunks) < processing_settings.image_concurrent_chunk_limit:
        original_chunks = chunks.copy()
        chunks = []
        chunk_id = 1
        
        # より細かく分割し直す
        micro_chunk_size = max(1, total_items // processing_settings.image_concurrent_chunk_limit)
    ...
    # ワーカー活用情報をログ出力
    expected_workers = min(total_chunks, processing_settings.image_concurrent_chunk_limit)
    utilization_rate = (expected_workers / processing_settings.image_concurrent_chunk_limit) * 100
...
def save_job_info(job_id: str, job_data: Dict[str, Any]) -> bool:
    ...
    try:
        ...
        redis_client.setex(key, celery_settings.image_job_timeout, value)
