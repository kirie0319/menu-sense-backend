...
class OCRTester:
    ...
    def create_sample_test_cases(self):
        ...
        sample_info = {
            ...
            "setup_instructions": [
                ...
                "3. python test_ocr_precision.py --run-tests でテストを実行してください"
            ]
        }
    ...
    async def benchmark_test(self, image_path: str, iterations: int = 5):
        """ベンチマークテスト（同じ画像を複数回実行）"""
        if not os.path.exists(image_path):
            print(f"❌ 画像ファイルが見つかりません: {image_path}")
            return
            
        print(f"🏃‍♂️ ベンチマークテスト: {image_path} ({iterations}回実行)")
        print("-" * 50)
        ...
        print(f"📈 最速/最遅: {min_time:.2f}s / {max_time:.2f}s")
...
async def main():
    ...
    parser.add_argument("--benchmark", type=str, help="ベンチマークテスト用画像")
    parser.add_argument("--iterations", type=int, default=5, help="ベンチマーク実行回数")
    ...
    if args.setup:
        ...
        await tester.benchmark_test(args.benchmark, args.iterations)
    elif args.run_tests:
        ...
        print("=" * 40)
        print("使用方法:")
        print("  python test_ocr_precision.py --setup              # テストセットアップ")
        print("  python test_ocr_precision.py --quick image.jpg    # クイックテスト")
        print("  python test_ocr_precision.py --benchmark image.jpg # ベンチマークテスト")
        print("  python test_ocr_precision.py --run-tests          # 包括的テスト")
        print()
